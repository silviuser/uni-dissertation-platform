    ...
    <>
      ...
      <Card>
        {myRequests && myRequests.length > 0 ? (
              ...
              <div key={req.id} style={{
                ...
                <div style={{ fontWeight: 600, marginBottom: 8, fontSize: '14px' }}>
                  {req.session?.universitySession?.name && req.session?.professor?.fullName
                    ? `${req.session.universitySession.name} - ${req.session.professor.fullName}`
                    : `Cerere #${req.id.slice(0, 6).toUpperCase()}`
                  }
                </div>
          ...
          <div className="meta">Nu ai cereri încă. Caută sesiuni și aplică pentru a începe.</div>
        )}
      </Card>
    </>
  );
